...
$(function(){
    // 把三个类型的input分别先获取
    var $theadInput = $('table thead input[type=checkbox]');  //表头中的全选
    var $tbodytInputs = $('table tbody input[type=checkbox]');  //表格中的每一行的选择框
     ...
     * 2.给表格中每一行数据的input选择框，赋值为表头的选中状态（true|false）
     * 3.给计算总价的全选框赋值为表头的选中状态（true|false）
     */
    // 表头全选
    $theadInput.change(function(){
        var checkState = $(this).prop('checked'); //获取全选框中的选中状态
        $tbodytInputs.prop('checked',checkState);  //把状态给表格中的选择框
    ...
    /*
        计算总价的全选
            1.给计算机总价的全选按钮 绑定点击事件，获取选择状态（true|false）
            2.把状态给表头的全选
            3.把状态给表格中的选择框
    */
    $totalPriceInput.change(function(){
        ...
        $tbodytInputs.prop('checked',checkState); //赋值给表格中全选框
    })
    ...
    /*
        表格中的选择框 反过来影响两个全选框
            ...
            3.循环表格中的选择框
            4.获取每一个选择框的选中状态
                判断：如果有一个是false 那么不是全选 flag = false;
            5.把flag的值赋值给两个全选框（因为flag就是对应选中状态）
     */
    $tbodytInputs.change(function(){  //给表格中单选框绑定事件
        var flag = true;  //定标杆为true
        $tbodytInputs.each(function(index,input){  //循环表格input
            var checkState = $(this).prop('checked');  //获取选中状态
            if(!checkState){  //如果有一个等于false
                flag = false;  //标杆变为false（全选状态为false）
            }
        })
        $theadInput.prop('checked',flag);  //把状态赋值给头部全选框
        $totalPriceInput.prop('checked',flag);  //把状态赋值给计算机总价全选框
     ...
     */
    $('.add').click(function(){
        var count = parseInt($(this).next().val());  //给增加绑定事件
        count++;  //自增
    ...
    $('.reduce').click(function(){  //绑定事件
        ...
        $(this).prev().val(count);  //把自减的值重新赋值给前面的输入框
    ...
    /**
        总计功能实现：（头部全选、计算总价全选、表格的选择框 + - 删除 六个地方调用总计）
            1.定义一个变量 用于保存总价 再定义一个变量 用于保存已选商品的数量
            1.获取所有表格中选择框，循环，获取选中状态，判断
            2.如果选中，那么就要累加这一行的小计
     */
    function allTotal(){
        var allPrice = 0;  //定义一个变量 用于保存总价
        var selectedCount = 0;  //定义一个变量 用于保存已选商品的数量
        ...
        // 渲染
        $('.total').text(allPrice.toFixed(2));  //渲染总加价
    ...
    function getGoodsCount(){
        // 获取数量
        var gooodsCount = $('table tbody tr').length;
        // 渲染
        $('.goodsCount').text(gooodsCount);
    }
